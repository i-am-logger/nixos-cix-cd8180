...
**Build**:
```bash
# Use this as a template for your own flake.nix
...
The Orange Pi 6 Plus board module uses the vendor kernel by default.

```nix
# Default: vendor kernel with GPU/NPU/ISP/VPU/GPIO drivers
# (automatically configured by orangepi6plus module)

# To override with mainline kernel:
...
```nix
# Requires vendor kernel

hardware.opengl = {
  enable = true;
  driSupport = true;
  # package = pkgs.cix-gpu-umd;  # TODO: when drivers are accessible
};
```

### Enable NPU (28.8 TOPS AI Accelerator)

```nix
# Requires vendor kernel

# TODO: Add when drivers are accessible
# environment.systemPackages = [ pkgs.cix-npu-umd ];
```
...
# Build to verify
nix build .#nixosConfigurations.orangepi6plus.config.system.build.toplevel

# Check what will be built
nix build .#nixosConfigurations.orangepi6plus.config.system.build.sdImage --dry-run
```
