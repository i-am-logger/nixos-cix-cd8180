...

        {
          ...
          environment.systemPackages = with nixpkgs.legacyPackages.aarch64-linux; [
            ...
            (python3.withPackages (ps: with ps; [
              ...
              matplotlib
              jupyter
              # TODO: Add NPU-specific packages when available
              # tensorflow
              # pytorch
              # onnx
            ]))

            # NPU drivers and tools (when available)
            # TODO: Uncomment when drivers are accessible
            # pkgs.cix-npu-umd
          ...
          users.users.ml.extraGroups = [ "docker" ];
