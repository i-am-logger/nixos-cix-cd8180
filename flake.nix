  ...
  outputs =
    ...
    in
    {
      packages = {
        ...
        sdImage-cross = self.nixosConfigurations.orangepi6plus-cross.config.system.build.sdImage;

        # Network Boot - Single package with all components
        netboot = pkgs.runCommand "netboot-orangepi6plus" { } ''
          mkdir -p $out
        ...
        netboot-cross = pkgs.runCommand "netboot-orangepi6plus-cross" { } ''
          mkdir -p $out
